...
P2P.prototype._startBcoin = function(callback) {
  ...
  if (['livenet', 'live', 'main', 'mainnet'].indexOf(this.node.network) !== -1) {
    network = 'main';
    port = this._configPeers[0].port || 7933;
  } else if (this.node.network !== 'regtest') {
    network = 'testnet';
    port = this._configPeers[0].port || 17933;
  } else {
    network = this.node.network;
    port = this._configPeers[0].port || 47944;
  }
...
P2P.prototype._startBcoinIfNecessary = function(callback) {
  if (!this._hasPeers()) {
    log.info('Peers not explicitly configured, starting a local bcoin node [ WARNING - Not ready for BTCP ].');
